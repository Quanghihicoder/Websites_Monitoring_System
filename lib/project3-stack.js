...
class Project3Stack extends Stack {
  ...
  constructor(scope, id, props) {
    ...
    // The pipeline downloads the code from the GitHub repository, 
    // initiates the Build and Test action using CodeBuild, and securely saves the built artifact on the S3 bucket.
    ...
    const buildProject = new codebuild.PipelineProject(this, 'project3-cicd-build-project', {
      buildSpec: codebuild.BuildSpec.fromSourceFilename('../buildspec.json'),
      environment: {
    ...
    const testProject = new codebuild.PipelineProject(this, 'project3-cicd-test-project', {
      buildSpec: codebuild.BuildSpec.fromSourceFilename('../buildspec-test.json'),
      environment: {
        buildImage: codebuild.LinuxBuildImage.STANDARD_7_0
      },
    ...
    // Deploy
    const deployProject = new codebuild.PipelineProject(this, 'project3-cicd-deploy-project', {
      buildSpec: codebuild.BuildSpec.fromSourceFilename('../buildspec-deploy.json'),
      environment: {
        buildImage: codebuild.LinuxBuildImage.STANDARD_7_0
      },
      projectName: 'project3-cicd-deploy-project'
    });
