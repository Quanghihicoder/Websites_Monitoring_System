...
class PipelineStack extends Stack {
  ...
  constructor(scope, id, props) {
    ...
    const assetsPublishingPermissions = new iam.PolicyStatement({
      ...
      resources: [
        `arn:aws:iam::${props.env.account}:role/cdk-${DefaultStackSynthesizer.DEFAULT_QUALIFIER}-file-publishing-role-${props.env.account}-${props.env.region}`
      ],
    });
    ...
    const deployBetaAction = new codepipeline_actions.CloudFormationCreateUpdateStackAction({
      ...
      templatePath:  buildArtifacts.atPath(`${props.betaApplicationStack.stackName}.template.json`),
    })
    ...
    const deployProdAction = new codepipeline_actions.CloudFormationCreateUpdateStackAction({
        ...
        adminPermissions: true,
        templatePath:  buildArtifacts.atPath(`${props.prodApplicationStack.stackName}.template.json`),
    })

    // Deploy Actions for multiple regions
    // const deployBetaActions = props.betaApplicationStacks.map(stack => new codepipeline_actions.CloudFormationCreateUpdateStackAction({
    //   actionName: `DeployBeta-${stack.stackName}`,
    //   stackName: stack.stackName,
    //   adminPermissions: true,
    //   templatePath: buildArtifacts.atPath(`${stack.stackName}.template.json`),
    // }));

    // const deployProdActions = props.prodApplicationStacks.map(stack => new codepipeline_actions.CloudFormationCreateUpdateStackAction({
    //   actionName: `DeployProd-${stack.stackName}`,
    //   stackName: stack.stackName,
    //   adminPermissions: true,
    //   templatePath: buildArtifacts.atPath(`${stack.stackName}.template.json`),
    // }));
